<?php
    ...
    switch ($whichTable) {
        //建立SQL語法
        case 'USER':
            $sql = "SELECT u.ID, u.ACCOUNT, u.NICKNAME, u.MOBILE , u.BIRTH , u.CREATE_TIME, us.USER_STATUS_NAME, l.LOGIN_TYPE
            FROM USER AS u
                JOIN USER_STATUS AS us
                ON u.USER_STATUS_ID = us.ID
                        JOIN LOGIN_TYPE AS l
                            ON u.LOGIN_TYPE_ID = l.ID
                                WHERE u.ACCOUNT_TYPE_ID = 1 AND u.USER_STATUS_ID != 3 AND u.ID = :whichID
                                    ORDER BY u.ID DESC;";
            break;
